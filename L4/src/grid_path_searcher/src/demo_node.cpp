...
{   
    ...
    map_vis.header.frame_id = "world";  // 删除“/”
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                for(int step=0 ; step<=_time_step ; step ++){
                    ...
                    */
                    ...
                    // v = at + v0
                    vel(0) = acc_input(0) * (double)step * delta_time + start_velocity(0);
                    vel(1) = acc_input(1) * (double)step * delta_time + start_velocity(1);
                    vel(2) = acc_input(2) * (double)step * delta_time + start_velocity(2);

                    // p = 1/2 * a * t^2 + v0 * t + p0
                    pos(0) = acc_input(0) * pow((double)step * delta_time, 2) / 2.0 +
                             start_velocity(0) * (double)step * delta_time + start_pt(0);
                    pos(1) = acc_input(1) * pow((double)step * delta_time, 2) / 2.0 +
                             start_velocity(1) * (double)step * delta_time + start_pt(1);
                    pos(2) = acc_input(2) * pow((double)step * delta_time, 2) / 2.0 +
                             start_velocity(2) * (double)step * delta_time + start_pt(2);
