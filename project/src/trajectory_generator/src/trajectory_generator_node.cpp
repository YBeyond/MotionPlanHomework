...
void visPath(MatrixXd nodes);    // A*检索的路径或者RDP简化的路径
void trajOptimization(Eigen::MatrixXd path);
void rcvOdomCallback(const nav_msgs::Odometry::ConstPtr &odom);  // 实时位姿信息
void rcvWaypointsCallback(const nav_msgs::Path &wp);   // 起点终点位置
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 &pointcloud_map);  // 障碍物点云
...
void execCallback(const ros::TimerEvent &e) {
  ...
  case GEN_NEW_TRAJ: {
    bool success = trajGeneration();    // !生成全局路径
    if (success)
...
VectorXd timeAllocation(MatrixXd Path) {
  VectorXd time(Path.rows() - 1);
      ...
      /*
         use "trapezoidal velocity" heuristic for time allocation
    */
  const double constAccDisplacement = ((_Vel * _Vel) / (2.0 * _Acc));
  int K = time.rows();
